...
struct BitcoinSignedMessageTests {
    ...
    func addressVerification() throws {
        ...
        for network in [BitcoinNetwork.mainnet, .testnet] {
            let compressedAddress = LegacyAddress(
                publicKey: key.publicKey,
                network: network,
                compressed: true
            )
            let uncompressedAddress = LegacyAddress(
                publicKey: key.publicKey,
                network: network,
                compressed: false
            )
    ...
    func verificationMismatches() throws {
        ...
        let address = LegacyAddress(publicKey: key.publicKey, network: .mainnet)
        let otherAddress = LegacyAddress(publicKey: otherKey.publicKey, network: .mainnet)
    ...
    func exactStringBytes() throws {
        ...
        let address = LegacyAddress(publicKey: key.publicKey, network: .testnet)
    ...
    @Test("all legacy header values parse and adjacent boundaries are rejected")
