...
struct AddressTests {
    ...
    func deterministicRoundTrips() throws {
        ...
        for network in [BitcoinNetwork.mainnet, .testnet] {
            let original = Address(publicKeyHash: hash, network: network)
            let text = original.description
            let decoded = try Address(text)
    ...
    func decodingPolicy() {
        ...
        #expect(throws: KeyFormatError.invalidPayloadByteCount(20)) {
            try Address(Base58Check.encode([0x00] + [UInt8](repeating: 1, count: 19)))
        }
        #expect(
            throws: KeyFormatError.invalidEncoding(
            ...
            try Address(Base58Check.encode([0x00] + [UInt8](repeating: 1, count: 21)))
        }
        #expect(throws: KeyFormatError.unsupportedVersion(0x05)) {
            try Address(Base58Check.encode([0x05] + [UInt8](repeating: 1, count: 20)))
        }
        #expect(throws: KeyFormatError.invalidEncoding(.checksumMismatch)) {
            try Address("1MirQ9bwyQcGVJPwKUgapu5ouK2E2Ey4gY")
        }
    }
}
