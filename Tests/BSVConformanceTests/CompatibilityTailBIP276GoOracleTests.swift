...
@Suite("Compatibility-tail BIP-276 Go conformance", .serialized)
struct CompatibilityTailBIP276GoOracleTests {
        ...
        case .available(let client):
            ...
            for (index, value) in cases.enumerated() {
                ...
                let decoded = try client.request(
                    id: "compatibility-tail-bip276-decode-\(index)",
    ...
    func prefixDomainDeviation() throws {
        ...
        case .available(let client):
            ...
            for (index, prefix) in ["A", "a_b", "a.b"].enumerated() {
                ...
                let encoded = try client.request(
                    id: "compatibility-tail-bip276-prefix-encode-\(index)",
                    operation: "script.bip276.encode",
                    arguments: [
                ...
                let decoded = try client.request(
                    id: "compatibility-tail-bip276-prefix-decode-\(index)",
                    operation: "script.bip276.decode",
    ...
    func adapterRejections() throws {
        ...
        case .available(let client):
            ...
            let encode = try client.request(
                id: "compatibility-tail-bip276-32769-encode",
                operation: "script.bip276.encode",
                arguments: [
                    "prefix": .string("x"),
                    "version": .string("1"),
            ...
            let decode = try client.request(
                id: "compatibility-tail-bip276-32769-decode",
                operation: "script.bip276.decode",
            ...
            for (id, version, network, category) in [
                ...
                let response = try client.request(
                    id: "compatibility-tail-bip276-zero-\(id)",
                    operation: "script.bip276.encode",
                    arguments: [
                        "prefix": .string("x"),
            ...
            for (id, text, category) in [
                ...
                let response = try client.request(
                    id: "compatibility-tail-bip276-zero-decode-\(id)",
                    operation: "script.bip276.decode",
                    arguments: ["text": .string(text)]
                )
                #expect(!response.ok)
