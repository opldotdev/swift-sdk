...
public enum BitcoinSignedMessageError: Error, Equatable, Sendable {
    ...
    /// The compact-signature header was outside the BSM range `27...34`.
...
/// A compatibility Bitcoin Signed Message compact signature.
///
/// New applications should use BRC-77 `SignedMessage` from `BSVMessage`.
public struct BitcoinMessageSignature: Hashable, Sendable {
    ...
    /// Parses an exact BSM wire signature (`header || r || s`).
    ...
    /// Verifies the message against an address on that address's network.
    public func verifies(message: [UInt8], address: Address) throws -> Bool {
...
/// Compatibility Bitcoin Signed Message framing, signing, and verification.
///
/// New applications should use BRC-77 `SignedMessage` from `BSVMessage`.
public enum BitcoinSignedMessage {
    ...
    /// Signs an exact byte-oriented message using BSM compact framing.
    ...
    /// Verifies padded Base64 against the exact UTF-8 bytes and supplied address.
