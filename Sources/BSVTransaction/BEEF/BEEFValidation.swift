...
extension BEEF {
    ...
    /// Computes and cross-checks every marked BUMP root by block height.
    public func merkleRootsByBlockHeight() throws -> [UInt32: Hash256] {
        ...
        for (index, path) in merklePaths.enumerated() {
            for transactionID in Self.markedTransactionIDs(in: path) {
                let root: Hash256
                do {
                ...
                if let existing = roots[path.blockHeight], existing != root {
                    throw BEEFError.conflictingMerkleRoot(blockHeight: path.blockHeight)
                }
                roots[path.blockHeight] = root
            }
        }
        return roots
    }
